...
- **InteractiveCharacter** (`client/src/components/character/InteractiveCharacter.tsx`): State-machine-driven interactive character with 6 states: idle (floating + periodic random swing/cast), swing (weapon attack animation), cast (magic particles + glow burst), sleep (tilts sideways with Zzz bubbles, auto-triggers at 23:00-06:00), waking (eye rub effect on click during sleep), attack (angry emote + lunge + full-screen glass crack overlay). All timers tracked in refs and cleaned on unmount. Used on home page.
- **AnimatedCharacter** (`client/src/components/character/AnimatedCharacter.tsx`): Simpler non-interactive wrapper with floating animation. Available but not currently used on home page (replaced by InteractiveCharacter).
